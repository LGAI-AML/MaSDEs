...
class masdes(nn.Module):
    ...
    def drift(self, t, X, Z):  # B x 1, B x D, B x D
        if self.args.sde_drift == "mckean_vlasov":
            out = torch.mean(Z, dim=0).unsqueeze(0) - Z
        elif self.args.sde_drift == "vanilla":
            out = Z
        return out
    
    def diffusion(self, t, X, Y):  # B x 1, B x D, B x 1
        if self.args.sde_diffusion == "constant":
            Y = torch.clamp(Y, min=self.args.sigma_low, max=self.args.sigma_low)
        elif self.args.sde_diffusion == "fcn":
            Y = torch.clamp(Y, min=self.args.sigma_low, max=self.args.sigma_high)
        return torch.diag_embed(Y).to(self.device)

    def liouville(self, X0, drift, diffusion, dt, dWt):
        # Simulate SDE with Euler-Maruyama
        X1_ = X0 + drift * dt + torch.squeeze(
                                    torch.matmul(diffusion, dWt.unsqueeze(-1)),
                                    dim=-1)
            
        # Estimate score of transition density p(1, x_1 | 0, x_0)
        score = self.score_function(X1_, X0, drift, diffusion, dt)

        drift_transformed = drift - 0.5 * torch.squeeze(
                                            torch.matmul(diffusion.pow(2), score.unsqueeze(-1)),
                                            dim=-1)
        return drift_transformed

    def score_function(self, X1, X0, drift, diffusion, dt):
        Sigma = diffusion * diffusion
        invSigma = torch.diag_embed(1 / torch.diagonal(Sigma, dim1=-2,dim2=-1))
        
        score = -(1/dt) * torch.squeeze(
            torch.matmul(invSigma, (X1 - X0 - drift*dt).unsqueeze(-1)), 
            dim=-1)
        return score.detach()
    ...
    def sde_propagation(self, X, mask, player_index):
        ...
        for t_gap in range(1, self.args.T_p - player_initial_time):
            ...
            if self.args.de_type == "ode":
                ...
                pf_ode = self.liouville(X0, drift_, diffusion_, dt, dWt)
                X1 = X0 + pf_ode * dt
            elif self.args.de_type == "sde":
                #### euler maruyama SDE ####
                X1 = X0 + drift_ * dt + torch.squeeze(
                    torch.matmul(diffusion_, dWt.unsqueeze(-1)),dim=-1)
                
            t0, W0, X0, M0 = t1, W1, X1, M1
    ...
    def fictitious_play(self, t, W, Y, M):
        ...
        for A_ in tqdm(range(self.args.A), desc='Agent', mininterval=0.1):  
            ...
            non_cooper_cost = self.non_cooper_cost(A_, total_weight_A)
            ...
            loss_train += (running_cost + terminal_cost + non_cooper_cost).item()  
            loss += running_cost + terminal_cost + non_cooper_cost
    ...
    def non_cooper_cost(self, A, weight_A):
        if self.args.cooperation is not True and any([A == agent for agent in self.args.non_coop_agent]):
            loss_non_cooper = (weight_A[0, :, :, 0] - 1).pow(2).mean()
        else:
            loss_non_cooper = 0.
        return loss_non_cooper
